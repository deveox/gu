...
// SyncMap is a generic sync.Map.
type AsyncMap[K any, V any] struct {
	m sync.Map
}

func (m *AsyncMap[K, V]) Delete(key K) { m.m.Delete(key) }
...
func (m *AsyncMap[K, V]) Store(key K, value V) { m.m.Store(key, value) }
